...
[ -n "${SomvillaChat_NEW_GROUP}" ] && {
	# App group IDs
	sed -i "" "s/group.${SomvillaChat_ORIG_GROUP}.app/group.${SomvillaChat_NEW_GROUP}.app/g" "ios/SomvillaChat Share/SomvillaChat Share.entitlements"
	sed -i "" "s/group.${SomvillaChat_ORIG_GROUP}.app/group.${SomvillaChat_NEW_GROUP}.app/g" "ios/Runner/Runner.entitlements"
	sed -i "" "s/group.${SomvillaChat_ORIG_GROUP}.app/group.${SomvillaChat_NEW_GROUP}.app/g" "ios/Runner.xcodeproj/project.pbxproj"
	# Bundle identifiers
	sed -i "" "s/${SomvillaChat_ORIG_GROUP}.app/${SomvillaChat_NEW_GROUP}.app/g" "ios/Runner.xcodeproj/project.pbxproj"
}

[ -n "${SomvillaChat_NEW_TEAM}" ] && {
	# Code signing team
	sed -i "" "s/${SomvillaChat_ORIG_TEAM}/${SomvillaChat_NEW_TEAM}/g" "ios/Runner.xcodeproj/project.pbxproj"
}
cat << EOHELP
...
2. Fix it yourself
    ...
    - Ensure that Xcode created the App ID successfully (for SomvillaChat.app and SomvillaChat.app.SomvillaChat-Share)
...
[ -n "${SomvillaChat_INSTALL_IPA}" ] && {
  ...
  cfgutil --foreach install-app "${TMPDIR}/SomvillaChat.ipa"
  rm -rf "${TMPDIR}"
}
